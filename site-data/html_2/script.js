...
*/
      ...
      // END EXTENSION INSTRUMENT PANEL
        ...
        $.ajax({ url: 'data/receiver.json',
         ...
         // BM TEST init map without feeder
         //   initialize_map(); // TEST
        // END BM TEST init map without feeder
}
...
function refreshSelected() {
	...
	refreshPageTitle();
        ...
        // GP REPLACEMENT
        gp_obj.RefreshSelectedGpHook(selected);

        if (!selected) { // no selected Data
                return;
        }
      
/*   GP Extension - DISABLED original code
 ...
 */
    // GP REPLACEMENT
    gp_obj.setSelectedInfoBlockVisibilityGpHook(planeSelected, mapIsVisible);
}

// Reposition selected plane info box if it overlaps plane marker
function adjustSelectedInfoBlockPosition() {
        return; // BM extension - Disable this function
