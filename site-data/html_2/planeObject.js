...
// BM EXTENSION - grab data from the sqlite DB 
    // check for a config setting 'DefaultAircraftDB = false' in order to use the sqLite DB
    if ( (typeof DefaultAircraftDB !== 'undefined') && ( DefaultAircraftDB===false ) ) {
      this.getAircraftData(); // use the extended one
    }
    else {
    // ORIGINAL CODE
    // request metadata via dbloader.js
	...
	this.last_message_time = receiver_timestamp - data.seen;
        ...
        if ('lat' in data && 'lon' in data) {
                ...
                if (SitePosition !== null) {
                        ...
                        this.sitedist = ol.sphere.getDistance(SitePosition, this.position, 6378137); //20190824 OL5.x update
                }
